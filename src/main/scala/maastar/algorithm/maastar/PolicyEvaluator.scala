...
class PolicyEvaluator() {
  ...
  private def allTransitionRewards(
    ...
    getAllAgentObservationCombinations(
      agentObservations.map { case (agent, obs) => agent -> obs.keys.toSet})
      .map { agentObs =>
  ...
  private def getAgentObsProbability(
      agentObs: Map[Agent, Set[Observation]],
      agentObservationProbabilities: Map[Agent, Map[Observation, Double]]): Double = {
    agentObservationProbabilities.map { case (agent, observationProbabilities) =>
